...
export default function LocationManager() {
  ...
  const handleAddCategory = () => {
    ...
    toast({
      ...
      description: `${newCatName} added to database.`,
    });
  };
  ...
  const handleAddLocation = () => {
    ...
    toast({
      ...
      description: `${newLocName} added to category.`,
    });
  };
  ...
  const handleDeleteCategory = (id: string, e: React.MouseEvent) => {
    ...
    if (PREDEFINED_IDS.includes(id)) {
      toast({
        title: "Action Denied",
        description: "Core protocols cannot be deleted.",
        variant: "destructive",
      });
      playSound("error");
      return;
  ...
  const toggleCategory = (
    cat: { id: string; locations: string[] },
    e: React.MouseEvent
  ) => {
    e.stopPropagation();
    ...
    if (cat.locations.length === 0) {
      toast({
        ...
        description: "Cannot enable empty category. Add locations first.",
        variant: "destructive",
      });
      playSound("error");
      return;
    ...
    if (
      state.settings.selectedCategories.includes(cat.id) &&
  ...
  const handleRemoveLocation = (
    ...
    if (isPredefined) {
      toast({
        title: "Action Denied",
        description: "Core locations cannot be removed.",
        variant: "destructive",
      });
      playSound("error");
      return;
    ...
    dispatch({
      ...
      payload: { categoryId: catId, location: loc },
    });
  };
      ...
      <div className="space-y-6 pb-24">
        ...
        <div className="grid gap-3">
          {state.gameData.categories.map((cat) => {
            const isSelected = state.settings.selectedCategories.includes(
              cat.id
            );
                  ...
                  <div className="p-4 pt-0 bg-black/20 border-t border-white/5">
                    <div className="grid grid-cols-2 gap-2 mt-4">
                      {cat.locations.map((loc, idx) => {
                        ...
                        const originalCat = INITIAL_CATEGORIES.find(
                          (c) => c.id === cat.id
                        );
                        ...
                        return (
                          <div
                            key={idx}
                            className="flex items-center justify-between p-2 rounded bg-white/5 text-sm group"
                          >
                            <span className="truncate mr-2">{loc}</span>
                            {!isCoreLoc ? (
                              <button
                                onClick={() =>
                                  handleRemoveLocation(cat.id, loc, false)
                                }
                      ...
                      <Dialog open={isAddingLoc} onOpenChange={setIsAddingLoc}>
                        <DialogTrigger asChild>
                          <Button
                            variant="outline"
                            className="border-dashed border-white/20 text-muted-foreground hover:text-primary hover:border-primary"
                          >
