...
export default function Voting() {
  ...
  const handleVote = () => {
    if (selectedPlayerId) {
      ...
      const activeSpies = state.players.filter(
        (p) => !p.isDead && p.role === "spy"
      );
      const spiesAfterVote =
        target?.role === "spy"
      ...
      if (target?.role === "spy" && spiesAfterVote >= 1) {
        toast({
          title: t("voting.spyCaught"),
          description: t("voting.spyRemain"),
        });
      }
  ...
  return (
    <div className="flex flex-col h-full py-6 space-y-6">
      <div className="text-center space-y-2">
        <h2 className="text-2xl font-bold font-mono text-red-500 uppercase tracking-widest animate-pulse">
          Emergency Meeting
        </h2>
      ...
      <div className="flex-1 overflow-y-auto space-y-3 px-1">
        {livingPlayers.map((player) => (
          ...
          >
            ...
            <div className="text-left flex-1">
              <p
                className={cn(
                  ...
                  selectedPlayerId === player.id && "text-red-400"
                )}
              >
                {player.name}
              </p>
            ...
            <Fingerprint
              className={cn(
                "w-5 h-5 opacity-0 group-hover:opacity-50 transition-opacity",
                selectedPlayerId === player.id && "opacity-100 text-red-500"
              )}
            />
          </button>
        ))}
